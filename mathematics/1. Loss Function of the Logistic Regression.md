...
\end{cases}
```
Intuitively, $\hat{y}^{y} (1 - \hat{y})^{1 - y}$ really does implement the above piecewise function. Simpler, yet not without magic. Why not have something simpler like:  
...
The explanation rests on the fact that while both of them are valid, the former one - $\hat{y}^{y} (1 - \hat{y})^{1 - y}$ is more convenient mathematically for succeeding steps. In fact, (3) is also known as an _additive indicator form of the Bernoulli probability_ and is not technically invalid, just useless in our objectives.  
