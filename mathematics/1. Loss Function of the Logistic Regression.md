...
## Problem - Where did equation (1) come from ?
```math
(1)\quad
L(\hat{y}, y) = -\left( y \log(\hat{y}) + (1 - y)\log(1 - \hat{y}) \right)
```
In machine learning mathematics, it is standard to see equation (1) for the loss of the logistic regression:
  
It is easy to understand that it's the logarithm of the function $P(y \mid x) = \hat{y}^{y} (1 - \hat{y})^{1 - y}$, given $P(y=1 \mid x) = \hat{y}$, with which we are taking in order to simplify the proceeding calculus needed for training.  

Where did the equation for $P(y \mid x)$ come from ?  
  
A common explanation for this states that this comes from the _Bernoulli conditional probability_ of which variables predicted through standard logistic regression satisfy.  
